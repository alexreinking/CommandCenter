...
bool GPSDecoder::decodeByte(int8_t newByte)
{
	...
	{//Lat+Lon comes in form AA.BBC*  A degrees B minutes C decimal minutes  
	 //Lat+Lon retruned in form AA.B* A degrees B decimal degrees decimal fixed between A and B
		char* decimal;
		int temp, temp2;
		double endPart;
		
		temp = (int)strtol(latitudeString, &decimal, 10); // AABB
		temp2 = (int)strtol(&latitudeString[5], &decimal, 10); //CCC
		//					(AABB		-		AA00)		    + 0.CCCC	 	= BB.CCC		
		endPart = (double)temp - (double)floor((double)temp/100)*100 + (double)temp2/10000;
		//					AA00000				+ BB.CCC/60
		latitude = 100000*floor((double)temp/100)  + 100000*endPart/60;
		if  (NSlatitudeString[0] == 'S')  {
			latitude = -latitude;
		}
		
		for (int i=0;i<10;i++) printf("%c",longitudeString[i]);
		
		temp = (int)strtol(longitudeString, &decimal, 10); // AABB
		temp2 = (int)strtol(&longitudeString[6], &decimal, 10); //CCC
		//					(AABB		-		AA00)		    + 0.CCCC	 	= BB.CCC		
		endPart = (double)temp - (double)floor((double)temp/100)*100 + (double)temp2/1000;
		//					AA00000				+ BB.CCC/60
		longitude = 100000*floor((double)temp/100)  + 100000*endPart/60;
		if (EWlongitudeString[0] == 'E') {
				longitude = -longitude;
		}
		
		/*
		cout << "Temp: " << temp << " T2: " << temp2 << endl;
		cout << "1: " << (double)temp - (double)floor((double)temp/100)*100 << " 3: " << (double)temp2/10000 << endl;
		cout << "First Part: " << floor((double)temp/100) << " Second: " << endPart/60 << endl;
		cout << "lon" << longitude << " End: " << endPart << endl;
	*/
	
		//temp = strtol(longitudeString, &decimal, 10) / 100;
